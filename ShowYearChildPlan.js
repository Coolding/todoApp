...
//本文件作用：显示年度任务下每一个目标任务的目标，方法，各项子目标等详细内容
...
componentWillMount() {
    ...
    let url="http://todoapp.applinzi.com/zljeurlzhdlqieut/getYearChildPlans/";
  ...
  modifyIfMonthChildPlay=(index)=>{
    let url="http://todoapp.applinzi.com/zuoewlzhflnqcur/modifyIfMonthChildPlay/";
    let formData=new FormData();      
    formData.append("ChildID",PlanDetail[index]['ChildID']); 
    fetch(url,{method:"POST",headers:{},body:formData}).then(response => response)
          .then(data =>{               
                PlanDetail[index]['ifMonthChildPlan']=Math.abs(PlanDetail[index]['ifMonthChildPlan']-1)
                this.forceUpdate();
            })
    ...
    return (
       <View  style={styles.container}>  
            <View style={styles.header}>  
                  <Text style={styles.headtitle}>年度：{this.state.ParentPlan}</Text> 
            </View>  
                                    ...
                                    <TouchableOpacity 
                                        ...
                                        <TouchableOpacity 
                                           style={{backgroundColor:'#BEB3F7',justifyContent: 'center',alignItems:'center',marginLeft:5,width:30,height:30,borderRadius:5,}}
                                            onPress={()=>this.modifyIfMonthChildPlay(index)}>
                                           <Text style={{color:'white',fontSize:20}}>{Item['ifMonthChildPlan']==1?"一":"+"}</Text>
                                       </TouchableOpacity>
                                    </TouchableOpacity>
